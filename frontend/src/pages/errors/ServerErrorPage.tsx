/**
 * VELTO — 500 Server Error Page
 */

import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import { ServerCrash, RefreshCw, Home, Zap } from 'lucide-react';
    ...
    <div className="min-h-screen bg-obsidian-950 flex flex-col justify-between p-6">
      ...
      <div className="max-w-md mx-auto text-center my-auto">
        <motion.div
          ...
          className="w-24 h-24 rounded-3xl bg-red-500/10 border border-red-500/20 flex items-center justify-center text-red-400 mx-auto mb-8 shadow-2xl shadow-red-500/10"
        >
        ...
        <motion.span
          ...
          className="text-red-400 font-black text-6xl tracking-widest block mb-2"
        >
          500
        </motion.span>
        ...
        >
          We encountered an unexpected server issue while processing your request. Our conversion engine engineers have been automatically notified.
        </motion.p>
        ...
        >
          <button onClick={() => window.location.reload()} className="w-full sm:w-auto">
            <Button variant="primary" icon={<RefreshCw className="w-4 h-4" />} fullWidth>
              Reload Page
            </Button>
          </button>
          <Link to="/" className="w-full sm:w-auto">
            <Button variant="secondary" icon={<Home className="w-4 h-4" />} fullWidth>
              Return Home
            </Button>
          </Link>
        </motion.div>
